...
Sema4Type isFullRxFIFO;
Sema4Type isEmptyRxFIFO;
Sema4Type mutexRxFIFO;
Sema4Type isFullTxFIFO;
Sema4Type isEmptyTxFIFO;
Sema4Type mutexTxFIFO;

// Initialize UART0
// Baud rate is 115200 bits/sec
void UART_Init(void){
	OS_InitSemaphore(&isFullRxFIFO,FIFOSIZE);
	OS_InitSemaphore(&isEmptyRxFIFO,0);
	OS_InitSemaphore(&mutexRxFIFO, 1);
	OS_InitSemaphore(&isFullTxFIFO,FIFOSIZE);
	OS_InitSemaphore(&isEmptyTxFIFO,0);
	OS_InitSemaphore(&mutexTxFIFO, 1);
  ...
  UART0_IBRD_R = 43;                     // IBRD = int(50,000,000 / (16 * 115,200)) = int(27.1267) for 80mhz 43.4027 ((DEFAULT OF 16MHz so 8.6806)
  ...
  GPIO_PORTA_AMSEL_R = 0;               // disable analog functionality on PA
                                        // UART0=priority 1
  NVIC_PRI1_R = (NVIC_PRI1_R&0xFFFF00FF)|0x00001000; // bits 13-15
...
char UART_InChar(void){
  char letter;
	OS_Wait(&isEmptyRxFIFO);
	OS_bWait(&mutexRxFIFO);
  while(RxFifo_Get(&letter) == FIFOFAIL){
		OS_Sleep(1);
	}
	OS_bSignal(&mutexRxFIFO);
	OS_Signal(&isEmptyRxFIFO);
	return(letter);
}
...
void UART_OutChar(char data){
	OS_Wait(&isFullTxFIFO);
	OS_bWait(&mutexTxFIFO);
	while(TxFifo_Put(data) == FIFOFAIL){
		OS_Sleep(1);
	}
	OS_bSignal(&mutexTxFIFO);
	OS_Signal(&isFullTxFIFO);
