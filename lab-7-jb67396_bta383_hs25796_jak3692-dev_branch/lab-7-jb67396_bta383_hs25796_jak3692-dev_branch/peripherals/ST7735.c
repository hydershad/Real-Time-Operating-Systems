...
uint32_t ST7735_DrawStringBackground(uint16_t x, uint16_t y, char *pt, int16_t textColor, int16_t bgColor){
  uint32_t count = 0;
  if(y>15) return 0;
  while(*pt){
...
void ST7735_OutUDecBackground(uint32_t n, int16_t bgColor){
  Messageindex = 0;
  fillmessage(n);
  Message[Messageindex] = 0; // terminate
  ST7735_DrawStringBackground(StX,StY,Message,StTextColor, bgColor);
  StX = StX+Messageindex;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,bgColor, 1);
  }
}
...
// Color set by ST7735_SetTextColor
// The string will not automatically wrap.
// inputs: inputs: string pointer to NULL-terminated ASCII string and integer value to print
// outputs: none
void ST7735_SplitScreen (){
	ST7735_FillRect(0, 72, 128, 88, 0x618C);
	ST7735_DrawFastHLine(0,73,128,0xFFFF);
}
//********ST7735_Message*****************
// Print a string of characters to the ST7735 LCD.
// Position determined by device and line
...
void ST7735_Message (int device, int line, char *string, int32_t value){
	  ...
	  int16_t bgColor = 0x0000;
		...
		line = line * 2;
		StY = line;
		//ST7735_FillRect(0, line * 8, 128, 11, bgColor);
		StX = ST7735_DrawStringBackground(0, line, string, StTextColor, bgColor);
		ST7735_OutUDecBackground(value, bgColor);
		OS_bSignal(&LCDmutex);
}
