...
void ST7735_Message(int device, int line, char* String, int32_t value){
	...
	if(device == 0){	//top of screen
			if(line >=0 && line < 4){		//if statements check to make sure correct bounds are established
					ST7735_DrawString(0, line*2, String, ST7735_YELLOW);
					ST7735_SetCursor(16, line*2);
	...
	if(device == 1){		//bottom of screen
			if(line >=0 && line < 4){		//if statements check to make sure correct bounds are established
					ST7735_DrawString(0, 9+(line*2), String, ST7735_YELLOW);
				ST7735_SetCursor(16, 9+(line*2));
					ST7735_OutUDec(value);
			}
			OS_bSignal(&LCDFree);
			return;
