...
unsigned static long prevTime;  // time at previous ADC sample
unsigned long newTime;         // time at current ADC sample
...
OS_Suspend();
newTime = OS_Time();
jitter = newTime-prevTime;
if(jitter>jmax){
	jmax = jitter;
}	
	UART_OutString("Testmain5 max Jitter in 1 uS resolution = ");
	UART_OutUDec(jmax);
