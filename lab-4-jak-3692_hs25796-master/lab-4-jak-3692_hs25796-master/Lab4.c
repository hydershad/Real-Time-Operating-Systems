...
//*******************lab 4 main **********
int realmain4(void){        // lab 4 real main
...
void TestDisk(void){  DSTATUS result;  unsigned short block;  int i; unsigned long n;
  ...
  for(block = 0; block < MAXBLOCKS; block++){
    ...
    if(eDisk_WriteBlock(lab4buffer,block))diskError("eDisk_WriteBlock",block); // save to disk
    PD3 = 0x00;     
  }  
  ...
  for(block = 0; block < MAXBLOCKS; block++){
    ...
    if(eDisk_ReadBlock(lab4buffer,block))diskError("eDisk_ReadBlock",block); // read from disk
    PD2 = 0x00;
    for(i=0;i<512;i++){
      n = (16807*n)%2147483647; // pseudo random sequence
      if(lab4buffer[i] != (0xFF&n)){
        printf("Read data not correct, block=%u, i=%u, expected %u, read %u\n\r",block,i,(0xFF&n),lab4buffer[i]);
        OS_Kill();
      }      
    }
  }  
  printf("Successful test of %u blocks\n\r",MAXBLOCKS);
  UART_OutString("eDisk successful");
...
void TestFile(void){   int i; char data; 
  ...
  UART_OutString("eFile test      ");
  // simple test of eFile
  if(eFile_Init())              diskError("eFile_Init",0); 
  if(eFile_Format())            diskError("eFile_Format",0); 
  eFile_Directory(List);
	UART_OutString(List);
  ...
  UART_OutString("eFile successful");
  Running=0; // launch again
  OS_Kill();
}
//************SW1Push2*************
