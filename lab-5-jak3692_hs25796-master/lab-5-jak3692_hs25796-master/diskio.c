...
void simpleformat(void){ DSTATUS result; uint16_t block; int i; uint32_t n; uint32_t errors = 0;
unsigned char buffer[512];
  // simple test of eDisk
  result = disk_initialize(0);  // initialize disk
  if(result) UART_OutString("disk_initialize error");
  n = 1;    // seed
  for(block = 0; block < MAXBLOCKS; block++){
    ...
    if(result) UART_OutString("disk_write error"); // save to disk
  }
  n = 1;  // reseed, start over to get the same sequence
  for(block = 0; block < MAXBLOCKS; block++){
    result = disk_read (0,buffer, block,1);
    if(result) UART_OutString("disk_read error");
    for(i=0; i<512; i++){
      n = (16807*n)%2147483647; // pseudo random sequence
      if(buffer[i] != (0xFF&n)){
        errors = errors + 1;
