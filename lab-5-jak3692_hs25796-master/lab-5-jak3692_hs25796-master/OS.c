...
int OS_AddProcess(void(*entry)(void), void *text, void *data, unsigned long stackSize, unsigned long priority){
	...
	for(i = 0; i < NUMTHREADS; i++){
		if(pcbs[i].id == 0){
			...
			pcbs[i].code = text;
			pcbs[i].data = data;
			pcbs[i].procThreads = 1;
			break;		
		}
	}
	if(i == NUMTHREADS){	//error
		EnableInterrupts();
		return 0;
	}
	if(OS_AddThread1(entry, 128, 1, i + 1)){
		EnableInterrupts();
		return 1;
	}
	else{
...
////////////////////////////////
  ...
  TIMER1_CFG_R = 0x00000004;    // 2) configure for 32-bit mode
  TIMER1_TAMR_R = 0x00000002;   // 3) configure for periodic mode, default down-count settings
  TIMER1_TAILR_R = 7999;    // 4) reload value
  TIMER1_TAPR_R = 0;            // 5) bus clock resolution
  TIMER1_ICR_R = 0x00000001;    // 6) clear TIMER1A timeout flag
  TIMER1_IMR_R = 0x00000001;    // 7) arm timeout interrupt
  NVIC_PRI5_R = (NVIC_PRI5_R&0xFFFF00FF)|0x00002000; // 8) priority 1	COME BACK
	  NVIC_EN0_R = 1<<21;           // 9) enable IRQ 21 in NVIC

	//TODO: ^^^^CHECK THIS IS CORRECT PRIORITY
...
Stacks[priority][i][STACKSIZE - 2] = (long)(task);
	...
	tcbs[priority][i].PCBid = RunPt->PCBid;
	if(tcbs[priority][i].PCBid > 0){
	pcbs[RunPt->PCBid - 1].procThreads += 1;
	}
	EndCritical(status);
	OS_EnableInterrupts();
return 1;
...
int OS_AddThread1(void(*task)(void), unsigned long stackSize, unsigned long priority, int id){
		//OS_DisableInterrupts();
//long status;
//status = StartCritical();
	int i;
...
void OS_Kill(void){		
	...
	if(RunPt->PCBid != 0){
		pcbs[RunPt->PCBid - 1].procThreads -=1;
		if(pcbs[RunPt->PCBid-1].procThreads == 0){
			pcbs[RunPt->PCBid - 1].id = 0;
		}
	}
	OS_EnableInterrupts();
	OS_Suspend();
...
unsigned long OS_Time(void){
	...
	int other = TIMER1_TAR_R;
	other = TIMER1_TAILR_R - other;
	other =  other/8;
