...
void URxFifo_Init(void){ long sr;
...
uint32_t volatile UTxPutI; // put next
uint32_t volatile UTxGetI; // get next
char static TxFifo[TXFIFOSIZE];

// initialize index FIFO
void UTxFifo_Init(void){ long sr;
  sr = StartCritical(); // make atomic
  UTxPutI = UTxGetI = 0;  // Empty
  EndCritical(sr);
}
// add element to end of index FIFO
// return FIFOSUCCESS if successful
int UTxFifo_Put(char data){
  if((UTxPutI-UTxGetI) & ~(TXFIFOSIZE-1)){
    return(FIFOFAIL); // Failed, fifo full
  }
  TxFifo[UTxPutI&(TXFIFOSIZE-1)] = data; // put
  UTxPutI++;  // Success, update
  return(FIFOSUCCESS);
}
// remove element from front of index FIFO
// return FIFOSUCCESS if successful
int UTxFifo_Get(char *datapt){
  if(UTxPutI == UTxGetI ){
    return(FIFOFAIL); // Empty if UTxPutI=UTxGetI
  }
  *datapt = TxFifo[UTxGetI&(TXFIFOSIZE-1)];
  UTxGetI++;  // Success, update
  return(FIFOSUCCESS);
}
// number of elements in index FIFO
// 0 to TXFIFOSIZE-1
uint32_t UTxFifo_Size(void){
 return ((uint32_t)(UTxPutI-UTxGetI));
}
...
void UART_Init(uint32_t priority){
  ...
  GPIO_PORTA_AMSEL_R = 0;               // disable analog functionality on PA
                                        // UART0=priority 7
  NVIC_PRI1_R = (NVIC_PRI1_R&0xFFFF00FF)|0x00038000; // bits 13-15
...
void static copyHardwareToSoftware(void){
  char letter;
  while(((UART0_FR_R&UART_FR_RXFE) == 0) && (URxFifo_Size() < (RXFIFOSIZE - 1))){
    letter = UART0_DR_R;
    URxFifo_Put(letter);
  }
}
...
char UART_InChar(void){
  char letter;
  while(UUUTxFifo_Get(&letter) == FIFOFAIL){};
  return(letter);
}
// input ASCII character from UART
...
char UART_InCharNonBlock(void){
  char letter;
  if(UUUTxFifo_Get(&letter) == FIFOFAIL){
    return 0; // no data
  }
  return(letter);
}
...
void NLO(void){		//new line output (NLO)
	UART_OutChar('\n');
	UART_OutChar(CR);
	UART_OutString("---> ");
}
