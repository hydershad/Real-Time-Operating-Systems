...
//Uses Timer 2A for Periodic Interrupt
//Timer 5 used for system clock
//Uses Timer 3A for debounce on port f
//Uses 0A for ADC on interpreter
//Uses 1A for ADC collect
//
//
//
int main(void){ 		//lab5 main
...
int heapstat = 	Heap_Init();
if(heapstat != HEAP_OK){
			ST7735_DrawString(0, 0, "Heap Init. Error", ST7735_Color565(0,0,255));
    while(1) {}
}
 ...
 int NumThreads = 0;		//keep track, stored in OS.h
  //create initial foreground threads
  NumThreads += OS_AddThread(&Idle,128,6); 
	NumThreads += OS_AddThread(&Interpret,128,2); 
...
//SDFILE MOUNT AND LOAD FUNCTIONS
...
int SDCmain(void){		//sdc main test
  UINT successfulreads, successfulwrites;
  uint8_t c, x, y;
